...
class Enumerator:
    ...
    def shuffsolv(self, domains, domain):
        ...
        os.system("shuffledns -strict-wildcard -wt 100 -r {3}/resolvers.txt -o {0} -v -list {1} -d {2}".format(path, domains, domain,self.resources))
        return
    ...
    def shuffledns(self):
        ...
        if(os.path.exists(output)):
            os.system("mv {0} {0}.old".format(output))
        os.system("shuffledns -strict-wildcard -wt 100 -r {2}/resolvers.txt -w {2}/subdomains.txt -o {0} -v -d {1}".format(output, domain, self.resources))
        self.shuffsolv(output, domain)
