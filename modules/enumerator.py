...
class Enumerator:
    ...
    def portenum(self):
        ...
        os.system("sudo NXScan --only-enumerate -l {0} -o {1} --naabu-param '-ports top-1000 -silent -rate 300 -t 7 -timeout 2000' ".format(subs,path+"/nxscan"))
    ...
    def shuffledns(self):
        ...
        if(os.path.exists(output)):
            os.system("mv {0} {0}.old".format(output))
        os.system("shuffledns -retries 2 -r {2}/resolvers.txt -w {2}/subdomains.txt -wt 100 -o {0} -v -d {1}".format(output, domain, self.resources))
        self.shuffsolv(output, domain)
